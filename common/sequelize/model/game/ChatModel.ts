...
@Table({
    ...
    indexes: [{
        name: 'index_chat',
        using: 'HASH',
        fields: ['chatType', 'sendUserId', 'reciverInfo'],
    }]
})
export class ChatModel extends Model {
    ...
    @Column({ type: DataType.TINYINT, comment: '参见chat.proto中的枚举', field: 'chatType', allowNull: false })
    chatType: number

    @Column({ type: DataType.STRING(16), comment: '发送者用户id', field: 'sendUserId', allowNull: true })
    sendUserId: string

    @Column({ type: DataType.STRING(16), comment: '接收者信息(null、服务器id)', field: 'reciverInfo', allowNull: true })
    reciverInfo: string
